...
class ExposedMappingProcessor(
    ...
    private fun generateTablesFile() {
        ...
        file.bufferedWriter().use { w ->
            ...
            w.appendLine("import dev.shaper.akdymall.features.common.database.ExpireTable")
            w.appendLine("import dev.shaper.akdymall.features.common.database.TimestampTable")
            w.appendLine()
            ...
            w.appendLine("      all.forEach { table ->")
            w.appendLine("          (table as? TimestampTable)?.run { createdAt; updatedAt }")
            w.appendLine("          (table as? ExpireTable)?.run { expiresAt }")
            w.appendLine("      }")
